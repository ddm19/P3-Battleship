...
public class BoardTest {
   ...
   * Cuando el Ship se ha hundido se comprueba que las
   ...
   * vistas */
	...
	public void testIsSeen2() {
		...
		for (int i=2; i<5; i++) {
				...
				assertTrue("x,y = 2,"+i,board.isSeen(new Coordinate(2,i)));
		}
		for (int i=1; i<4; i++)
			for (int j=1; j<6; j++) {
				assertTrue("x,y = "+i+","+j,board.isSeen(new Coordinate(i,j)));
			}
	}
		...
		for (int i=5; i<board.getSize(); i++) 
		  for (int j=5; j<board.getSize(); j++) {
		     if ( (i!=7) || (j<6) ||(j>8) ){
		    		 ...
		    		 assertEquals("x,y = "+i+","+j,CellStatus.WATER,board.hit(new Coordinate(i,j)));
		     }
		     else 
		    	 if ((i==7)&&(j==8))
			        assertEquals("x,y = "+i+","+j,CellStatus.DESTROYED,board.hit(new Coordinate(i,j)));
		    	 else 
		    		 assertEquals("x,y = "+i+","+j,CellStatus.HIT,board.hit(new Coordinate(i,j)));
		} 		
	}
	...
	public void testGetNeighborhoodShipOutOfBounds() {
		Set<Coordinate> neighborhood = board.getNeighborhood(galeon);
	...
	public void testGetNeighborhoodShipCompletelyOutOfBounds() {
		Set<Coordinate> neighborhood = board.getNeighborhood(galeon,new Coordinate(0,10));
		assertTrue(neighborhood.isEmpty());
	...
	Object getBoardField(Board board, String atribute) throws Exception {
        ...
        Field field = board.getClass().getDeclaredField(atribute);
